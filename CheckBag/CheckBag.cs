...
namespace CheckBag
{
    ...
    public class CheckBag : TerrariaPlugin
    {
        ...
        /// <param name="args"> 需要被清理的玩家 </param>
        public void ClearPlayersItem(TSPlayer args)
        {
            ...
            bool Check(List<ItemData> li, bool isCurrent)
            {
                ...
                if (data != null)
                {
                    ...
                    if (num < max)
                    {
                        ...
                        for (int i = 0; i < 22; i++)
                        {
                            args.TPlayer.buffType[i] = 0;
                        }
                        args.SendData(PacketTypes.PlayerBuff, "", args.Index, 0f, 0f, 0f, 0);
        ...
        #region 用5号包(PlayerSlot)缓存封禁数据，辅助ClearPlayersItem方法。
